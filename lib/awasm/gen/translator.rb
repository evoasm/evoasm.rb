...
require 'awasm/gen/enum'
require 'awasm/gen/name_util'
require 'awasm/gen/x64'
  ...
  module Gen
    class BaseTranslator
      ...
      def arch_var_name(indep_arch = false)
        "#{indep_arch ? '((awasm_arch *)' : ''}#{arch}#{indep_arch ? ')' : ''}"
      end
      ...
      def helper_to_c(expr)
        ...
        when :in?
          ...
          if !name.is_a?(Symbol)
            ...
            if name == :reg_code
              call_args[0] = "(awasm_#{arch}_reg_id) #{call_args[0]}" 
            end
    ...
    class FuncTranslator < BaseTranslator
      ...
      def emit_log(_state, level, msg, *exprs)
        ...
        msg = msg.gsub('%', '%" AWASM_PARAM_VAL_FORMAT "')
        io.puts %[awasm_#{level}("#{msg}" #{expr_part});]
      end
          ...
          emit_c_block "static void\n#{func_name}(#{arch_c_type} *#{arch_var_name},"\
            ...
            emit_c_block "for(i = 0; i < #{writes.size}; i++)" do
              emit_c_block "switch(#{table_var_name}[param_vals[order]][i])" do
                ...
                io.puts "default: awasm_assert_not_reached();"
              end
            end
          end
        end
    ...
    class Translator < BaseTranslator
      ...
      def load_enums
        ...
        when :x64
          ...
          @reg_types = Enum.new :reg_type, Awasm::Gen::X64::REGISTERS.keys, prefix: arch
          @operand_types = Enum.new :operand_type, Awasm::Gen::X64::Inst::OPERAND_TYPES, prefix: arch
          @reg_names = Enum.new :reg_id, Awasm::Gen::X64::REGISTER_NAMES, prefix: arch
        end
      end
      ...
      def inst_to_c(io, inst, params)
        ...
        io.indent do
          ...
          io.indent do
            ...
            io.puts '(awasm_inst_encode_func)' + inst_encode_func_name(inst), eol: ','
          end
          io.puts '},'
      ...
      def insts_to_c(io = StrIO.new)
        io.puts "static const awasm_x64_inst #{insts_var_name}[] = {"
          ...
          io.indent do
            params.each do |param|
              ...
              io.indent do
                ...
                io.puts '(awasm_domain *) &' + param_domain_var_name(param_domain)
              end
              io.puts '},'
            end
          end
      ...
      def param_domains_to_c(io = StrIO.new)
        ...
        io.puts "const uint16_t awasm_n_domains = #{registered_param_domains.size};"
