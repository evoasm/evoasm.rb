...
#define EVOASM_ASG_NULL_IDX (uint16_t) -1
...
evoasm_success
evoasm_asg_init(evoasm_asg *graph, uint16_t capa) {
...
evoasm_asg_link(evoasm_asg *graph, evoasm_asg_node *node_from, evoasm_asg_node *node_to, evoasm_asg_edge label) {
  ...
  uint16_t from = evoasm_asg_node_index(graph, node_from);
  uint16_t to = evoasm_asg_node_index(graph, node_to);
