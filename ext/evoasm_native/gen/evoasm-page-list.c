...
void
evoasm_page_list_clear(evoasm_page_list *free_list, uint16_t start, uint16_t end) {
...
evoasm_success
evoasm_page_list_init(evoasm_page_list *free_list, uint16_t capa) {
...
_evoasm_page_list_grow(evoasm_page_list *free_list) {
  uint16_t new_capa = free_list->capa + free_list->capa / 2;
...
evoasm_page_list_delete(evoasm_page_list *free_list, evoasm_page *e) {
  uint16_t idx = evoasm_page_list_index(free_list, e);
...
bool
evoasm_page_list_find(evoasm_page_list *free_list, evoasm_page *value, uint16_t *index) {

  if(free_list->len == 0) return false;

  for(uint16_t i = 0; i < free_list->capa; i++) {
