...
evoasm_success
evoasm_sym_tbl_init(evoasm_sym_tbl *tbl, uint32_t capa) {
  char *data = evoasm_malloc(sizeof(char) * capa);
  if(data == NULL) {
    return false;
  }
...
evoasm_success
evoasm_sym_tbl_get(evoasm_sym_tbl *tbl, const char *key, uint32_t len, evoasm_sym *sym) {
  unsigned i, j;
...
next:;
  ...
  if(tbl->len + len + 1 > tbl->capa) {
    ...
    char *new_data = evoasm_realloc(tbl->data, new_capa);
    if(new_data == NULL) {
